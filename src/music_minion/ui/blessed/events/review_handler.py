...
from music_minion.ui.blessed.state import (
    UIState,
    add_history_line,
    enter_review_confirm,
    exit_review_mode,
)


def handle_review_input(
    ctx: AppContext, ui_state: UIState, user_input: str
    ...
    if user_input.strip().lower() == "done":
        ...
        ui_state = add_history_line(
            ui_state, "🔄 Regenerating tags based on your feedback...", "yellow"
        )
        ...
        try:
            ...
            local_path_str = track_data.get("local_path")
            if not local_path_str:
                ui_state = add_history_line(
                    ui_state, "❌ Error: Missing track file path", "red"
                )
            ...
            new_tags = ai_review.regenerate_tags_with_feedback(
                track, conversation, initial_tags
            )
            ...
            if new_tags:
                ...
                ui_state = add_history_line(
                    ui_state, "📝 Updated Tags Preview:", "green"
                )
                ui_state = add_history_line(ui_state, "=" * 60, "white")
                for tag, reasoning in new_tags.items():
                    ui_state = add_history_line(
                        ui_state, f'  • {tag}: "{reasoning}"', "white"
                    )
                ui_state = add_history_line(ui_state, "=" * 60, "white")
                ui_state = add_history_line(ui_state, "", "white")
                ui_state = add_history_line(
                    ui_state, "💾 Save these tags? [y/n]:", "yellow"
                )
                ...
                ui_state = add_history_line(
                    ui_state, "❌ Failed to regenerate tags", "red"
                )
                ui_state = exit_review_mode(ui_state)
    ...
    try:
        ...
        track_local_path = track_data.get("local_path")
        db_track = get_track_by_path(track_local_path) if track_local_path else None
...
Please respond naturally to the user's feedback about the tags. If they suggest changes, acknowledge them and explain what you'll update. Be concise and helpful."""
        ...
        ui_state = add_history_line(
            ui_state, "Install with: pip install openai", "yellow"
        )
    except ai_module.AIError as e:
        ui_state = add_history_line(ui_state, f"❌ AI Error: {e}", "red")
    except Exception as e:
...
def handle_review_confirmation(
    ctx: AppContext, ui_state: UIState, user_input: str
) -> tuple[AppContext, UIState]:
    """
    ...
    if user_response == "y":
        ...
        try:
            ...
            track_local_path = track_data.get("local_path")
            db_track = get_track_by_path(track_local_path)
            if not db_track:
                ui_state = add_history_line(
                    ui_state, "❌ Track not found in database", "red"
                )
                ui_state = exit_review_mode(ui_state)
                return ctx, ui_state
            ...
            ui_state = add_history_line(
                ui_state, "✅ Tags saved successfully!", "green"
            )

            # Extract and save learnings
            ui_state = add_history_line(ui_state, "", "white")
            ui_state = add_history_line(
                ui_state, "🧠 Extracting learnings from conversation...", "yellow"
            )
            ...
            if learnings_summary:
                ai_module.append_to_learnings_section(
                    "Rules - Don't Tag These", learnings_summary
                )
                ui_state = add_history_line(ui_state, "✅ Learnings saved!", "green")
            else:
                ui_state = add_history_line(
                    ui_state, "⚠️  Could not extract learnings automatically", "yellow"
                )

        except ai_module.AIError as e:
            ui_state = add_history_line(
                ui_state, f"❌ AI Error saving tags: {e}", "red"
            )
        except Exception as e:
            ui_state = add_history_line(
                ui_state, f"❌ Unexpected error saving tags: {e}", "red"
            )
            import traceback
