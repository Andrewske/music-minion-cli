...
def handle_show_metadata_editor(
...
def handle_metadata_editor_enter(
...
def handle_metadata_editor_delete(
    ctx: AppContext, ui_state: UIState
) -> tuple[AppContext, UIState]:
    """
...
def handle_metadata_editor_add(
    ctx: AppContext, ui_state: UIState
) -> tuple[AppContext, UIState]:
    """
...
def handle_metadata_editor_save(
    ctx: AppContext, ui_state: UIState
) -> tuple[AppContext, UIState]:
    """
    ...
    if success:
        ...
        if track:
            ...
            ui_state = add_history_line(
                ui_state, f"✅ Metadata saved and reloaded", "green"
            )
            ...
            ui_state = hide_metadata_editor(ui_state)
            ui_state = add_history_line(
                ui_state, f"✅ Metadata saved (failed to reload)", "yellow"
            )
    else:
        ui_state = add_history_line(
            ui_state, f"❌ Failed to save metadata: {error_msg}", "red"
        )
...
def _handle_edit_field(
    ...
    if field_name in ["ratings", "notes", "tags"]:
        ...
        ui_state = set_editor_mode(
            ui_state, "list_editor", {"editing_field": field_name, "items": items}
        )
    else:
...
def _handle_delete_item(
    ...
    ui_state = set_editor_mode(
        ui_state, "list_editor", {"editing_field": field_type, "items": new_items}
    )
    ...
    ui_state = add_history_line(
        ui_state, f"⚠️  Marked {field_type[:-1]} for deletion", "yellow"
    )
...
def _handle_add_item(ctx: AppContext, ui_state: UIState) -> tuple[AppContext, UIState]:
    ...
    if field_type in ["notes", "tags"]:
        ...
        ui_state = replace(
            ...
            editor_input="",
            editor_data={**editor_data, "adding_item_type": field_type},
        )
        ...
        ui_state = add_history_line(
            ui_state, f"⚠️  Adding {field_type} not supported", "yellow"
        )
