...
def handle_play_track_from_viewer(
    ctx: AppContext, ui_state: UIState, playlist_id: int, track_index: int
) -> tuple[AppContext, UIState]:
    """
    ...
    if playlist_id < 0:
        ui_state = add_history_line(
            ui_state, f"❌ Invalid playlist ID: {playlist_id}", "red"
        )
        return ctx, ui_state

    if track_index < 0:
        ui_state = add_history_line(
            ui_state, f"❌ Invalid track index: {track_index}", "red"
        )
        return ctx, ui_state
    ...
    if track_index >= len(tracks):
        ui_state = add_history_line(
            ui_state, f"❌ Track index out of range: {track_index}", "red"
        )
        return ctx, ui_state
    ...
    ui_state = add_history_line(
        ui_state, f"▶️  Now playing: {artist} - {title}", "white"
    )
...
def handle_remove_track_from_playlist(
    ctx: AppContext, ui_state: UIState, track_id: int, playlist_name: str
) -> tuple[AppContext, UIState]:
    """
    ...
    try:
        ...
        ui_state = add_history_line(
            ui_state, f"✅ Removed track from playlist", "green"
        )
...
def handle_track_viewer_add_to_playlist(
    ctx: AppContext, ui_state: UIState, track_id: int
) -> tuple[AppContext, UIState]:
    """
...
def handle_track_viewer_remove(
    ctx: AppContext, ui_state: UIState, track_id: int
) -> tuple[AppContext, UIState]:
    """
...
def handle_track_viewer_edit_filters(
    ctx: AppContext, ui_state: UIState, playlist_id: int
) -> tuple[AppContext, UIState]:
    """
    ...
    if pl["type"] != "smart":
        ui_state = add_history_line(
            ui_state, f"❌ Cannot edit filters on manual playlist", "red"
        )
        return ctx, ui_state
...
def handle_track_viewer_like(
    ...
    # Add feedback
    ui_state = add_history_line(
        ui_state, f"👍 Liked: {library.get_display_name(library_track)}", "white"
    )
            ...
            try:
                ...
                if state.authenticated:
                    ...
                    if success:
                        ...
                        ui_state = add_history_line(
                            ui_state, "   ✓ Synced like to SoundCloud", "white"
                        )
                    else:
                        ui_state = add_history_line(
                            ui_state,
                ...
                ui_state = add_history_line(
                    ui_state, f"   ⚠ Error syncing to SoundCloud: {e}", "yellow"
                )
...
def handle_track_viewer_unlike(
    ctx: AppContext, ui_state: UIState, track_id: int
) -> tuple[AppContext, UIState]:
    """
    ...
    if not database.has_soundcloud_like(track_id):
        ui_state = add_history_line(
            ui_state, f"⚠ {library.get_display_name(library_track)}", "yellow"
        )
        ui_state = add_history_line(
            ui_state, "   Track is not liked on SoundCloud", "yellow"
        )
        return ctx, ui_state
    ...
    if not soundcloud_id:
        ...
        ui_state = add_history_line(
            ui_state, "   Track has like marker but no SoundCloud ID", "yellow"
        )
        return ctx, ui_state
    ...
    try:
        ...
        if not state.authenticated:
            ui_state = add_history_line(
                ui_state, "⚠ Not authenticated with SoundCloud", "yellow"
            )
            ui_state = add_history_line(
                ui_state, "   Run: library auth soundcloud", "yellow"
            )
            return ctx, ui_state
        ...
        if success:
            ...
            ui_state = add_history_line(
                ui_state,
                f"💔 Unliked on SoundCloud: {library.get_display_name(library_track)}",
                "white",
            )
            ui_state = add_history_line(
                ui_state, "   ✓ Removed like from SoundCloud", "white"
            )
            ui_state = set_feedback(ui_state, "✓ Unliked track", "✓")
        else:
            ui_state = add_history_line(
                ui_state, f"⚠ Failed to unlike on SoundCloud: {error_msg}", "yellow"
            )

    except Exception as e:
        ui_state = add_history_line(
            ui_state, f"⚠ Error unliking on SoundCloud: {e}", "yellow"
        )
