...
def handle_ai_setup_command(
    ...
    try:
        ...
        log(
            "   You can also set OPENAI_API_KEY environment variable or create .env in project root",
            level="info",
        )
...
def handle_ai_analyze_command(ctx: AppContext) -> tuple[AppContext, bool]:
    ...
    try:
        ...
        if result["success"]:
            ...
            if tags_added:
                log(
                    f"✅ Added {len(tags_added)} tags: {', '.join(tags_added)}",
                    level="info",
                )
            else:
            ...
            if usage:
                log(
                    f"   Tokens used: {usage.get('prompt_tokens', 0)} prompt + {usage.get('completion_tokens', 0)} completion",
                    level="info",
                )
                log(
                    f"   Response time: {usage.get('response_time_ms', 0)}ms",
                    level="info",
                )
        else:
...
def handle_ai_test_command(ctx: AppContext) -> tuple[AppContext, bool]:
    ...
    try:
        ...
        if test_results["success"]:
            ...
            log(
                f"   Track: {track_info.get('artist', 'Unknown')} - {track_info.get('title', 'Unknown')}",
                level="info",
            )
            log(
                f"   Generated tags: {', '.join(test_results.get('ai_output_tags', []))}",
                level="info",
            )

            token_usage = test_results.get("token_usage", {})
            log(
                f"   Tokens used: {token_usage.get('prompt_tokens', 0)} prompt + {token_usage.get('completion_tokens', 0)} completion",
                level="info",
            )
            log(
                f"   Response time: {token_usage.get('response_time_ms', 0)}ms",
                level="info",
            )
...
def handle_ai_review_command(ctx: AppContext) -> tuple[AppContext, bool]:
    ...
    log(
        f"🎵 Reviewing tags for: {library.get_display_name(current_track)}",
        level="info",
    )

    try:
        # Get or generate tags with reasoning
        tags_with_reasoning, is_new = ai_review.get_or_generate_tags_with_reasoning(
            current_track
        )
        ...
        log("", level="info")
        log(
            "💬 Entering conversation mode. Type your feedback, or 'done' to finish.",
            level="info",
        )
        log(
            '   Example: "This is half-time, not energetic. Don\'t tag key."',
            level="info",
        )
        log("", level="info")
...
def handle_ai_enhance_command(
    ...
    if args and args[0] != "prompt":
        log(
            f"❌ Unknown enhance subcommand: '{args[0]}'. Use: ai enhance prompt",
            level="error",
        )
        return ctx, True
