...
def propose_prompt_improvements(
    current_prompt: str, learnings: str
...
def test_prompt_on_tracks(
    prompt_text: str, num_tracks: int = 3
) -> list[dict[str, Any]]:
            ...
            results.append(
                {
                    ...
                    "old_tags": {
                        tag["tag_name"]: tag.get("reasoning", "No reasoning")
                        for tag in ai_tags
                    },
                    "new_tags": new_reasoning
                    or {tag: "Generated" for tag in new_tag_list},
                    "success": True,
                }
            )
...
def display_prompt_comparison(
    ...
    for i, result in enumerate(test_results, 1):
        ...
        if track_info.get("genre"):
            print(
                f"   Genre: {track_info['genre']}, BPM: {track_info.get('bpm', 'N/A')}, Key: {track_info.get('key', 'N/A')}"
            )
...
def enhance_prompt_interactive() -> bool:
    ...
    if len(content_lines) == 0:
        print(
            "⚠️  No learnings accumulated yet. Use 'ai review' to build up feedback first."
        )
        return False
    ...
    try:
        ...
        # Display comparison
        display_prompt_comparison(
            current_prompt, new_prompt, changes_summary, test_results
        )
        ...
        if approve == "y":
            ...
            print(
                f"📁 New prompt saved to: {save_prompt_version(new_prompt, changes_summary)}"
            )
            approved = True
            return True
        else:
        ...
        if not approved:
            set_active_prompt(
                original_active, "Reverted - proposal rejected or error occurred"
            )
