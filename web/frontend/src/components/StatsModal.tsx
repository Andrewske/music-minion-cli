...
export function StatsModal({ isOpen, onClose, playlistId = null }: StatsModalProps) {
  const isPlaylistMode = !!playlistId;
  const { data: globalStats, isLoading: isGlobalLoading, isError: isGlobalError, refetch: refetchGlobal } = useStats();
  const { data: playlistStats, isLoading: isPlaylistLoading, isError: isPlaylistError, refetch: refetchPlaylist } = usePlaylistStats(playlistId);
  const { data: playlistTracks, isLoading: isPlaylistTracksLoading, isError: isPlaylistTracksError } = usePlaylistTracks(playlistId);

  const isLoading = isPlaylistMode ? isPlaylistLoading : isGlobalLoading;
  const isError = isPlaylistMode ? isPlaylistError : isGlobalError;
  const refetch = isPlaylistMode ? refetchPlaylist : refetchGlobal;
    ...
    >
      <div className="relative max-w-6xl w-full mx-4 max-h-[90vh] bg-slate-950 border border-slate-800 rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="overflow-y-auto max-h-[90vh]">
          ...
          ) : isLoading || (!globalStats && !playlistStats) ? (
            <StatsSkeleton />
          ) : (
            <div className="p-6">
              {/* Header */}
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-slate-100 mb-2">
                  {isPlaylistMode && playlistStats ? `${playlistStats.playlist_name} Statistics` : 'Library Statistics'}
                </h2>
                <p className="text-slate-400">
                  {isPlaylistMode ? 'Track your playlist curation progress' : 'Track your music curation progress'}
                </p>
              </div>

              {/* Stat Cards Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {isPlaylistMode && playlistStats ? (
                  <>
                    <StatCard
                      icon="🎵"
                  ...
                  <>
                    <StatCard
                      icon="🎵"
                      value={globalStats.total_tracks}
                      label="Total Tracks"
                      subtitle="In your library"
                    />
                    <StatCard
                      icon="⚖️"
                      value={globalStats.total_comparisons}
                      label="Comparisons"
                      subtitle="Total ratings made"
                    />
                    <StatCard
                      icon="📊"
                      value={`${globalStats.coverage_percent.toFixed(1)}%`}
                      label="Coverage"
                      subtitle={`${globalStats.compared_tracks} tracks rated`}
                    />
                    <StatCard
                      icon="📈"
                      value={globalStats.average_comparisons_per_day.toFixed(1)}
                      label="Daily Average"
                      subtitle="Comparisons per day"
                    />
                    {globalStats.prioritized_tracks && globalStats.prioritized_coverage_percent !== null && globalStats.prioritized_coverage_percent !== undefined && (
                      <StatCard
                        icon="⭐"
                        value={`${globalStats.prioritized_coverage_percent.toFixed(1)}%`}
                        label="Priority Coverage"
                        subtitle={`${globalStats.prioritized_tracks} tracks prioritized`}
                      />
                    )}
                  </>
                ) : null}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                  {isPlaylistMode && playlistStats ? (
                    ...
                    <GenreChart genres={globalStats.top_genres} />
                  ) : null}
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                  {isPlaylistMode && playlistStats ? (
                    ...
                    <Leaderboard entries={globalStats.leaderboard} />
                  ) : null}
                </div>
              </div>

                {/* Additional Stats */}
                {!isPlaylistMode && globalStats && (
                  <div className="mt-6 space-y-3">
                    {globalStats.estimated_days_to_coverage && (
                      <div className="text-center">
                        <p className="text-slate-400">
                          Estimated {globalStats.estimated_days_to_coverage} days to reach full coverage
                          at current pace
                        </p>
                      </div>
                    )}
                    {globalStats.prioritized_estimated_days && (
                      <div className="text-center">
                        <p className="text-slate-400">
                          Estimated {globalStats.prioritized_estimated_days} days to reach full priority coverage
                          at current pace
                        </p>
                      </div>
                    )}
