...
def update_emoji_custom_name_mutation(
    emoji_id: str, custom_name: Optional[str], db_conn
) -> None:
...
def add_emoji_to_track(track_id: int, request: AddEmojiRequest) -> dict[str, bool]:
    """Add emoji to track. Returns {"added": true/false}."""
    emoji_id = normalize_emoji_id(request.emoji_id)
    with get_db_connection() as conn:
        added = add_emoji_to_track_mutation(
            track_id, emoji_id, conn, source_type="manual"
        )
    return {"added": added}


@router.delete("/emojis/tracks/{track_id}/emojis/{emoji_id}")
def remove_emoji_from_track(track_id: int, emoji_id: str) -> dict[str, bool]:
    """Remove emoji from track."""
    emoji_id = normalize_emoji_id(emoji_id)
    with get_db_connection() as conn:
        result = remove_emoji_from_track_mutation(track_id, emoji_id, conn)
    return {"removed": result["success"], "protected": result["protected"]}
...
def get_all_emojis(limit: int = 100, offset: int = 0) -> list[EmojiInfo]:
...
def update_emoji_metadata(
    emoji_id: str, request: UpdateEmojiMetadataRequest
) -> dict[str, bool]:
    """Update emoji custom name."""
    emoji_id = normalize_emoji_id(emoji_id)
    with get_db_connection() as conn:
...
def delete_custom_emoji(emoji_id: str) -> dict[str, bool]:
    ...
    with get_db_connection() as conn:
        ...
        cursor = conn.execute(
            "SELECT type, file_path FROM emoji_metadata WHERE emoji_id = ?", (emoji_id,)
        )
        row = cursor.fetchone()
        ...
        if row["type"] != "custom":
            raise HTTPException(
                status_code=400,
                detail="Only custom emojis can be deleted via this endpoint",
            )
