# Frontend API Client - Builder Network Layer

## Files to Create
- `web/frontend/src/api/builder.ts` (new)

## Implementation Details

Create TypeScript API client for builder endpoints using fetch API.
...
export const builderApi = {
  ...
  getSession: async (playlistId: number): Promise<SessionResponse> => {
...
All functions throw errors with descriptive messages that can be caught by React Query and displayed in toast notifications.

Common error responses:
- **400**: Invalid filters, smart playlist selected
- **404**: Playlist or session not found
- **500**: Server error

## Acceptance Criteria

1. API client exports all required functions
2. TypeScript interfaces match backend schemas
3. Proper error handling with descriptive messages
4. All HTTP methods correct (GET, POST, PUT, DELETE)
5. Request bodies properly serialized as JSON
6. Response parsing handles errors gracefully
7. API base URL configurable via environment variable

## Dependencies
- Task 03: Backend API routes must be implemented
...
await builderApi.startSession(1);  // Should return session data
await builderApi.getFilters(1);     // Should return empty array
```

Integration testing:
- Test with React Query in next task
- Error states handled by UI components
