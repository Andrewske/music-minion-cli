"""EXP6: top_level_artist as additional matching signal.

Base: Merged v3 (artist-weight + guarded substring + guarded lev)

EXP6 addition:
- Fetch top_level_artist from SC tracks (first artist in comma list; sometimes
  manually corrected to actual track artist vs label/reposter in artist field)
- Include top_level_artist tokens in TF-IDF index text
- When building SC artist tokens, use top_level_artist as primary artist signal
  in addition to the full artist field
- For artist-weighted scoring: if local artist matches top_level_artist but not
  artist, that counts as a strong positive signal (prevents label name mismatch)
- Tracks stats: how many SC tracks had top_level_artist, how many benefited

Run with: uv run python scripts/test_exp6_metadata_artist.py
"""
...
def sc_track_tokens(track: dict) -> tuple[set[str], set[str], set[str]]:
    ...
    EXP6: top_level_artist is included in sc_artist_tokens as the primary
    artist signal. This handles cases where 'artist' has label/multi-artist
    text but top_level_artist has the actual track creator.
    """
    title = track.get("title") or ""
    artist = track.get("artist") or ""
    top_artist = track.get("top_level_artist") or ""

    full_tokens = tokenize(f"{title} {artist}")
    real_title = parse_sc_real_title(title)
    real_tokens = tokenize(f"{real_title} {artist}")

    # EXP6: build sc_artist_tokens from both artist and top_level_artist
    sc_artist_tokens = tokenize(artist) if artist else set()
    if top_artist:
        sc_artist_tokens = sc_artist_tokens | tokenize(top_artist)

    # EXP6: include top_level_artist tokens in full token set so matching picks them up
    top_artist_tokens = tokenize(top_artist) if top_artist else set()
    combined = full_tokens | real_tokens | top_artist_tokens
...
def build_tfidf_index(sc_tracks: list[dict]) -> tuple[TfidfVectorizer, np.ndarray]:
    ...
    for t in sc_tracks:
        ...
        real_title = parse_sc_real_title(title)
        # EXP6: include top_level_artist so TF-IDF can surface primary artist
        top_artist = t.get("top_level_artist", "") or ""
        combined = normalize_text(f"{artist} {top_artist} {title} {real_title}")
...
def main() -> None:
    ...
    sc_rows = conn.execute("""
        SELECT id, title, artist, soundcloud_id, top_level_artist
        FROM tracks
    ...
    # EXP6: count how many SC tracks have top_level_artist
    sc_with_top_artist = sum(
        1 for t in sc_tracks
        if t.get("top_level_artist") and t["top_level_artist"] != (
            t.get("artist", "").split(",")[0].strip() if t.get("artist") else ""
        )
    )
    print(f"SC tracks with top_level_artist != first artist: {sc_with_top_artist} / {len(sc_tracks)}")

    print("\nMatching (exp6: v3 + top_level_artist as artist signal)...")
    ...
    print("=" * 60)
    print("CONFIDENCE DISTRIBUTION  [EXP6: top_level_artist]")
    print("=" * 60)
    ...
    for title_frag, artist_frag, note in spot_checks:
        ...
        for lt, sc, score, debug in results:
            ...
            if title_frag.lower() in lt_title and (artist_frag is None or artist_frag.lower() in lt_artist):
                print(format_sample(lt, sc, score, debug))
                baseline_s = score_v2(lt, sc) if sc else 0.0
                spot_results[key] = (lt, sc, score, baseline_s, debug)
                found = True
                break
        if not found:
            print("    NOT FOUND")
    ...
    print(f"\n{'=' * 60}")
    print("COMPARISON: v3 baseline vs exp6 (top_level_artist)")
    print("=" * 60)
    print(f"  {'Track':<45} {'v3':>6}  {'e6':>6}  {'delta':>7}  boosts")
    print(f"  {'-'*45} {'-'*6}  {'-'*6}  {'-'*7}  {'-'*10}")

    for title_frag, artist_frag, note in spot_checks:
        key = f"{title_frag}|{artist_frag or ''}"
        lt, sc, e6_s, baseline_s, debug = spot_results.get(key, (None, None, 0.0, 0.0, {}))
        label = f"{title_frag}" + (f" ({artist_frag})" if artist_frag else "")
        delta = e6_s - baseline_s
        boosts = []
        if debug.get("substr_boosted"):
            boosts.append("sub")
        if debug.get("lev_boosted"):
        ...
        print(f"  {label:<45} {baseline_s:>6.3f}  {e6_s:>6.3f}  {delta:>+7.3f}  {boost_str}")
