"""Strategy Merged v3: Combines best experiments with safety guards.

Base: Merged v2 (TF-IDF candidates + containment + synonym/remix/filename fixes)

Added from experiments:
1. EXP3: Artist-weighted containment (2x weight for artist tokens)
   - Guard: artist boost (1.1x) only fires when non-artist coverage > 0.5
2. EXP1: Exact title substring pre-filter
   - Guard: skip when any penalty < 1.0 (preserve correctness penalties)
   - Guard: require >=2 meaningful words in local title
3. EXP4: Levenshtein boost for mid-range scores
   - Guard: only use combined "artist - title" comparison, not title-only

Run with: uv run python scripts/test_strategy_merged_v3.py
"""
...
def main() -> None:
    ...
    print("\nMatching (v3: artist-weight + guarded substring + guarded lev)...")
    ...
    print("=" * 60)
    print("CONFIDENCE DISTRIBUTION  [MERGED V3]")
    print("=" * 60)
    ...
    for threshold, label in [(0.90, ">=0.90"), (0.80, ">=0.80"), (0.70, ">=0.70"), (0.50, ">=0.50")]:
        ...
        delta_str = f"  (v2: {bl:.1f}%, delta {pct - bl:+.1f}%)" if bl else ""
    ...
    print(f"\n{'=' * 60}")
    print("COMPARISON: v2 baseline vs v3 merged")
    print("=" * 60)
    print(f"  {'Track':<45} {'v2':>6}  {'v3':>6}  {'delta':>7}  boosts")
    print(f"  {'-'*45} {'-'*6}  {'-'*6}  {'-'*7}  {'-'*10}")
    ...
    for title_frag, artist_frag, note in spot_checks:
        ...
        print(f"  {label:<45} {v2_s:>6.3f}  {v3_s:>6.3f}  {delta:>+7.3f}  {boost_str}")
