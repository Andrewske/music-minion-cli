"""Experiment 1: Exact Title Substring Pre-filter

Based on test_strategy_merged_v2.py — adds an exact title substring check
BEFORE containment scoring to boost short-title tracks like LRAD, The Game, #SELFIE.

Changes vs baseline (merged_v2):
1. After TF-IDF candidates, for each SC candidate:
   - Clean+normalize local track title
   - Clean+normalize SC track title (also parse the real title from "Label - Artist - Title")
   - If cleaned local title is an exact substring of cleaned SC full title,
     OR cleaned SC real title is an exact substring of cleaned local title:
     boost via blend: final = max(containment_score, 0.5*containment_score + 0.5*1.0)
   - For very short titles (<=3 tokens): if exact substring match, use max(score, 0.85) as floor

Run with: uv run python scripts/test_exp1_substring.py
"""
...
def normalize_for_substring(s: str | None) -> str:
    """Normalize for exact substring matching: lowercase, strip punctuation except # and &,
    collapse whitespace. Produces a clean string suitable for 'in' checks."""
    if not s:
        return ""
    s = s.lower()
    # Keep # and & but remove other punctuation (no space replacement — strip entirely)
    s = re.sub(r"[^\w\s&#]", "", s)
    s = re.sub(r"\s+", " ", s)
    return s.strip()
...
def containment_score_with_penalty(
    ...
    Critical token penalty: if ANY critical token is missing → 0.5x
    Noise penalty: if sc_tokens >> local_tokens → 0.85x
    FIX 2 — Reverse remix penalty: SC has remix but LOCAL doesn't → 0.5x
    """
    if not local_tokens:
        return 0.0, {"base": 0.0, "critical_penalty": 1.0, "noise_penalty": 1.0, "reverse_remix_penalty": 1.0, "used_sc_tokens": sc_tokens, "intersection": set(), "missing_critical": set()}
...
# ---------------------------------------------------------------------------
# EXP 1: Exact Title Substring Pre-filter helpers
# ---------------------------------------------------------------------------

def clean_local_title_for_substring(track: dict) -> str:
    """Produce a clean normalized local title for substring matching.

    Strips artist info — only uses the title field (not artist, not filename).
    """
    title = track.get("title") or ""
    return normalize_for_substring(title)


def clean_sc_titles_for_substring(sc_track: dict) -> tuple[str, str]:
    """Return (full_normalized, real_title_normalized) for SC track.

    Parses "Label - Artist - Real Title" format for real_title.
    """
    title = sc_track.get("title") or ""
    full_cleaned = strip_bracket_suffixes(title)
    full_norm = normalize_for_substring(full_cleaned)

    real_title = parse_sc_real_title(title)
    real_norm = normalize_for_substring(real_title)

    # Also build artist+title combo for substring check
    artist = sc_track.get("artist") or ""
    artist_norm = normalize_for_substring(artist)
    full_with_artist = normalize_for_substring(f"{artist} {full_cleaned}")

    return full_norm, real_norm, full_with_artist, artist_norm
...
def substring_boost(
    ...
    """Apply substring boost if local title is found in SC title or vice versa.

    Returns (boosted_score, was_boosted).

    Boost rules:
    - If substring match found: blend = max(score, 0.5*score + 0.5*1.0)
    - For very short local titles (<=3 meaningful tokens from title only):
      if substring match: use max(score, 0.85) as floor
    """
    if not local_title_norm:
        return containment_score, False

    sc_full_norm, sc_real_norm, sc_full_with_artist, _ = clean_sc_titles_for_substring(sc_track)

    # Check: local title substring of SC full title or SC real title
    local_in_sc = (
        local_title_norm in sc_full_norm
        or local_title_norm in sc_real_norm
        or local_title_norm in sc_full_with_artist
    )

    # Check: SC real title is an exact substring of local title
    sc_in_local = bool(sc_real_norm and sc_real_norm in local_title_norm)

    found = local_in_sc or sc_in_local

    if not found:
        return containment_score, False

    # Count title-only tokens (exclude noise, articles) to determine "short title"
    # Use tokenize on just the title to get meaningful token count
    title_str = local_title_norm
    title_tok_count = len([
        w for w in title_str.split()
    ...
    score = max(containment_score, blended)

    if title_tok_count <= 3:
...
# ---------------------------------------------------------------------------
# Main matching function (EXP 1 version)
# ---------------------------------------------------------------------------
...
def match_track(
    ...
    """Find best SC match using TF-IDF + containment scoring + substring boost.

    Returns (best_sc_track, final_score, debug_info).
    """
    local_tokens, critical_tokens = local_track_tokens(local_track)
    local_title_norm = clean_local_title_for_substring(local_track)
    ...
    for idx, tfidf_s in candidates:
        ...
        # EXP 1: Apply substring boost
        boosted_score, was_boosted = substring_boost(
            score, local_tokens, local_title_norm, sc
        )
        ...
        if boosted_score > best_score:
            ...
            best_debug = {
                "tfidf": tfidf_s,
                "score": boosted_score,
                "containment_pre_boost": score,
                "substring_boosted": was_boosted,
...
def format_sample(lt: dict, sc: dict | None, score: float, debug: dict) -> str:
    ...
    pre_boost = debug.get("containment_pre_boost", score)
    boosted = debug.get("substring_boosted", False)

    penalty_str = f"crit_pen={cp:.1f} noise_pen={np_:.2f} rev_remix_pen={rrp:.1f}"
    boost_str = f" [SUBSTR BOOST pre={pre_boost:.3f}]" if boosted else ""
    lines = [
...
# ---------------------------------------------------------------------------
# Baseline scoring (merged_v2 equivalent) — for spot-check delta
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    print("\nMatching local tracks → SC (EXP 1: substring boost)...")
    ...
    total = len(results)
    print("=" * 60)
    print("CONFIDENCE DISTRIBUTION  [EXP 1: substring boost]")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("CUMULATIVE COUNTS  [baseline: 71.3%@0.90, 76.3%@0.80]")
    print("=" * 60)
    ...
    # Count how many tracks were substring-boosted
    boosted_count = sum(1 for _, _, _, debug in results if debug.get("substring_boosted", False))
    print(f"\n  Substring-boosted tracks: {boosted_count} ({boosted_count / total * 100:.1f}%)")
    ...
    # --- Spot checks ---
    print(f"\n{'=' * 60}")
    print("SPOT CHECKS  [EXP 1 — look for LRAD, The Game, #SELFIE changes]")
    print("=" * 60)
    ...
    # --- COMPARISON: baseline merged_v2 vs exp1 ---
    print(f"\n{'=' * 60}")
    print("COMPARISON: baseline (merged_v2) vs EXP 1 (substring boost)")
    print("=" * 60)

    # Baseline scores at the major thresholds (hardcoded from merged_v2 run)
    BASELINE_90 = 71.3
    BASELINE_80 = 76.3

    exp1_90 = sum(1 for _, _, score, _ in results if score >= 0.90) / total * 100
    exp1_80 = sum(1 for _, _, score, _ in results if score >= 0.80) / total * 100

    print(f"  {'Threshold':<12} {'Baseline':>10}  {'EXP 1':>10}  {'Delta':>8}")
    print(f"  {'-'*12} {'-'*10}  {'-'*10}  {'-'*8}")
    print(f"  {'>=0.90':<12} {BASELINE_90:>9.1f}%  {exp1_90:>9.1f}%  {exp1_90 - BASELINE_90:>+7.1f}%")
    print(f"  {'>=0.80':<12} {BASELINE_80:>9.1f}%  {exp1_80:>9.1f}%  {exp1_80 - BASELINE_80:>+7.1f}%")

    print(f"\n  {'Track':<45} {'v1':>6}  {'exp1':>6}  {'delta':>7}  boost?")
    print(f"  {'-'*45} {'-'*6}  {'-'*6}  {'-'*7}  {'-'*6}")

    for title_frag, artist_frag, note in spot_checks:
        key = f"{title_frag}|{artist_frag or ''}"
        lt, sc, exp1_s, v1_s, debug = spot_results.get(key, (None, None, 0.0, 0.0, {}))
        label = f"{title_frag}" + (f" ({artist_frag})" if artist_frag else "")
        delta = exp1_s - v1_s
        delta_str = f"{delta:+.3f}"
        boosted_flag = "YES" if (debug and debug.get("substring_boosted")) else "no"
        print(f"  {label:<45} {v1_s:>6.3f}  {exp1_s:>6.3f}  {delta_str:>7}  {boosted_flag}")
