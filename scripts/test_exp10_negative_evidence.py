"""EXP10: Negative evidence penalty for discriminating tokens.

Base: Merged v3 (TF-IDF candidates + containment + artist-weight + guarded substring + guarded lev)

Added EXP10: Negative evidence penalty
- After containment score, compute extra token ratio:
  tokens in SC real-title but NOT in local track / total SC real-title tokens
- Artist-name tokens that are "extra" count 2x (different artist is very bad)
- Noise words and common words don't count as extra discriminating tokens
- Penalty formula: multiply by (1.0 - 0.3 * extra_ratio), capped at 0.7x minimum
- Conditions to apply: >=2 meaningful extra tokens AND containment < 0.95

Guards:
- Skip when containment >= 0.95 (near-perfect matches untouched)
- Only count tokens from SC real title (not full title with label prefix)
- Noise words and article words don't trigger the penalty

Run with: uv run python scripts/test_exp10_negative_evidence.py
"""
...
# ---------------------------------------------------------------------------
# EXP3: Artist-weighted containment scoring (with guard)
# ---------------------------------------------------------------------------
...
def containment_score_with_penalty(
    ...
    # --- Artist-weighted base score (EXP3) ---
    ...
    # --- Artist boost with guard (EXP3 + fix) ---
    # Only apply 1.1x when non-artist token coverage > 0.5
    artist_boost = 1.0
...
# ---------------------------------------------------------------------------
# EXP10: Negative evidence penalty
# ---------------------------------------------------------------------------

def negative_evidence_penalty(
    containment_s: float,
    local_tokens: set[str],
    local_artist_tokens: set[str],
    sc_real_tokens: set[str],
    ...
    """Penalize when SC real-title has meaningful discriminating tokens not in local track.

    Guards:
    - Skip when containment >= 0.95 (near-perfect, don't touch)
    - Only count tokens from SC real title (not label-prefix noise)
    - Noise words and article words don't count as discriminating
    - Need >=2 meaningful extra tokens to apply any penalty

    Artist extra tokens count 2x because a different artist name is strong
    negative evidence (e.g. "Boss Mode" by Knife Party vs "LRAD" by Knife Party:
    "boss" and "mode" are extra SC title words).
    """
    debug: dict = {
        "neg_extra_tokens": set(),
        "neg_extra_ratio": 0.0,
        "neg_penalty": 1.0,
        "neg_applied": False,
    }

    if containment_s >= 0.95:
        return containment_s, debug

    # Only consider meaningful SC real-title tokens (no noise/article)
    sc_real_meaningful = {
        tok for tok in sc_real_tokens
        if tok not in NOISE_WORDS and tok not in ARTICLE_WORDS
    }

    if not sc_real_meaningful:
        return containment_s, debug

    # Find tokens in SC real title NOT present in local track tokens
    extra_tokens = sc_real_meaningful - local_tokens

    # Filter out any tokens that are too short or digits (shouldn't exist after
    # tokenize but be defensive)
    extra_tokens = {t for t in extra_tokens if len(t) >= 2 and not t.isdigit()}

    if len(extra_tokens) < 2:
        return containment_s, debug

    # Weight: artist tokens that are "extra" count 2x
    # sc_artist_tokens that are extra = very strong negative evidence
    extra_artist = extra_tokens & sc_artist_tokens
    extra_non_artist = extra_tokens - sc_artist_tokens

    weighted_extra = len(extra_artist) * 2 + len(extra_non_artist)
    weighted_total = sum(
        2 if tok in sc_artist_tokens else 1
        for tok in sc_real_meaningful
    )

    if weighted_total == 0:
        return containment_s, debug

    extra_ratio = weighted_extra / weighted_total

    # Apply penalty: multiply by (1.0 - 0.3 * extra_ratio), floor at 0.7x
    penalty = max(0.70, 1.0 - 0.3 * extra_ratio)
    penalized = containment_s * penalty

    debug = {
        "neg_extra_tokens": extra_tokens,
        "neg_extra_ratio": extra_ratio,
        "neg_penalty": penalty,
        "neg_applied": True,
        "neg_extra_artist": extra_artist,
        "neg_extra_non_artist": extra_non_artist,
    }
    return penalized, debug


# ---------------------------------------------------------------------------
# EXP1: Substring boost (with guards)
# ---------------------------------------------------------------------------
...
def substring_boost(
    ...
    """Apply substring boost only when safe.

    Guards:
    - Skip when any penalty < 1.0 (don't override correctness signals)
    - Require >=2 meaningful words in local title
    """
...
# ---------------------------------------------------------------------------
# EXP4: Levenshtein boost (combined-string only)
# ---------------------------------------------------------------------------
...
def levenshtein_boost(
    ...
    """Levenshtein boost using combined "artist - title" only (no title-only)."""
    if score >= 0.90:
        return score, 0.0
...
def match_track(
    ...
    for idx, tfidf_s in candidates:
        ...
        # EXP10: Negative evidence penalty — applied to raw containment score
        containment_after_neg, neg_debug = negative_evidence_penalty(
            containment_s, local_tokens, local_artist_tokens,
            sc_real_tokens, sc_artist_tokens,
        )

        # EXP1: Substring boost — only when no penalties fired
        # Also guard: don't substring-boost when neg evidence fired
        penalties_clean = (
            score_debug["critical_penalty"] >= 1.0
            and score_debug["reverse_remix_penalty"] >= 1.0
            and not neg_debug["neg_applied"]
        )
        score_after_substr, was_substr_boosted = substring_boost(
            containment_after_neg, local_title_norm, sc, penalties_clean,
        )
        ...
        if final_score > best_score:
            ...
            best_debug = {
                ...
                "containment_after_neg": containment_after_neg,
                ...
                "neg_applied": neg_debug["neg_applied"],
                "neg_extra_tokens": neg_debug.get("neg_extra_tokens", set()),
                "neg_extra_ratio": neg_debug.get("neg_extra_ratio", 0.0),
                "neg_penalty": neg_debug.get("neg_penalty", 1.0),
                "neg_extra_artist": neg_debug.get("neg_extra_artist", set()),
                "neg_extra_non_artist": neg_debug.get("neg_extra_non_artist", set()),
            }
...
def format_sample(lt: dict, sc: dict | None, score: float, debug: dict) -> str:
    ...
    neg_applied = debug.get("neg_applied", False)
    neg_ratio = debug.get("neg_extra_ratio", 0.0)
    neg_penalty = debug.get("neg_penalty", 1.0)
    neg_extra_tokens = debug.get("neg_extra_tokens", set())
    cont_after_neg = debug.get("containment_after_neg", cont)

    penalty_str = f"crit={cp:.1f} noise={np_:.2f} rrp={rrp:.1f} ab={ab:.1f}"
    if neg_applied:
        penalty_str += f" NEG={neg_penalty:.2f}(r={neg_ratio:.2f})"
    ...
    lines = [
        f"  [{score:.3f}] base={base:.3f} {penalty_str}{boost_str}",
        f"    LOCAL: {local_label}",
        f"    SC:    {sc_label}",
    ...
    if neg_applied:
        lines.append(f"    neg_extra={sorted(neg_extra_tokens)}  cont_before={cont:.3f}  cont_after={cont_after_neg:.3f}")
    return "\n".join(lines)


# ---------------------------------------------------------------------------
# Baseline v2 scoring for comparison
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    print("\nMatching (EXP10: v3 + negative evidence penalty)...")
    ...
    for i, lt in enumerate(local_tracks):
        ...
        if debug.get("lev_boosted"):
            lev_count += 1
        if debug.get("neg_applied"):
            neg_count += 1

    total = len(results)
    print(f"  {total}/{total} (100%) — done")
    print(f"  Substring boosts: {substr_count} ({substr_count / total * 100:.1f}%)")
    print(f"  Levenshtein boosts: {lev_count} ({lev_count / total * 100:.1f}%)")
    print(f"  Negative evidence penalties: {neg_count} ({neg_count / total * 100:.1f}%)\n")
    ...
    print("=" * 60)
    print("CONFIDENCE DISTRIBUTION  [EXP10: negative evidence]")
    print("=" * 60)
    ...
    print(f"\n{'=' * 60}")
    print("COMPARISON: v2 baseline vs EXP10")
    print("=" * 60)
    print(f"  {'Track':<45} {'v2':>6}  {'exp10':>6}  {'delta':>7}  flags")
    print(f"  {'-'*45} {'-'*6}  {'-'*6}  {'-'*7}  {'-'*15}")

    for title_frag, artist_frag, note in spot_checks:
        key = f"{title_frag}|{artist_frag or ''}"
        lt, sc, v3_s, v2_s, debug = spot_results.get(key, (None, None, 0.0, 0.0, {}))
        ...
        if debug.get("artist_boost", 1.0) > 1.0:
            flags.append("art")
        if debug.get("neg_applied"):
            flags.append(f"NEG({debug.get('neg_penalty', 1.0):.2f})")
        flag_str = "+".join(flags) if flags else "-"
        print(f"  {label:<45} {v2_s:>6.3f}  {v3_s:>6.3f}  {delta:>+7.3f}  {flag_str}")

    # --- Neg evidence penalty examples ---
    print(f"\n{'=' * 60}")
    print("NEG EVIDENCE: tracks that got penalized (sample)")
    print("=" * 60)
    penalized = [(lt, sc, score, debug) for lt, sc, score, debug in results if debug.get("neg_applied")]
    print(f"  Total penalized: {len(penalized)}")
    print(f"  Showing top 15 by penalty ratio (most discriminating extra tokens):\n")
    penalized_sorted = sorted(penalized, key=lambda x: x[3].get("neg_extra_ratio", 0.0), reverse=True)
    for lt, sc, score, debug in penalized_sorted[:15]:
        print(format_sample(lt, sc, score, debug))
