"""EXP7: Token order similarity via sequential bigram overlap.

Base: Merged v3 (artist-weight + guarded substring + guarded lev)

Added EXP7:
4. Bigram order boost — extract ordered consecutive word pairs from
   normalized title+artist text; compare local bigrams vs SC full-title
   bigrams AND SC real-title bigrams.
   - For scores 0.60-0.89 (uncertain range): if bigram_overlap > 0.5,
     blend 80/20 (score / bigram_overlap) as bonus.
   - For scores 0.90+: use bigrams as tie-breaker only (never lower score).
   - "Bass Head" vs "Head Bass" will now differ in bigram signal.

Run with: uv run python scripts/test_exp7_bigram_order.py
"""
...
# ---------------------------------------------------------------------------
# EXP7: Bigram order similarity
# ---------------------------------------------------------------------------

def extract_bigrams(text: str) -> list[tuple[str, str]]:
    """Extract ordered consecutive word pairs from normalized text.

    Filters out noise/article words and digits, same as tokenize().
    e.g. "knife party boss mode" → [("knife","party"), ("party","boss"), ("boss","mode")]
    """
    normalized = normalize_text(text)
    words = [
        ...
        and w not in ARTICLE_WORDS
    ]
    return [(words[i], words[i + 1]) for i in range(len(words) - 1)]


def bigram_overlap_ratio(local_bigrams: list[tuple[str, str]], sc_bigrams: list[tuple[str, str]]) -> float:
    """Jaccard-style overlap: |intersection| / |union| for bigram lists."""
    if not local_bigrams and not sc_bigrams:
        return 1.0
    if not local_bigrams or not sc_bigrams:
        return 0.0
    local_set = set(local_bigrams)
    sc_set = set(sc_bigrams)
    intersection = local_set & sc_set
    union = local_set | sc_set
...
def bigram_boost(
    ...
    """Apply bigram order signal.

    - 0.60-0.89 (uncertain): if overlap > 0.5, blend 80/20 score/overlap as bonus.
    - 0.90+: use as tie-breaker only — never lower score.

    Returns (new_score, bigram_overlap, was_boosted_flag as 1.0/0.0).
    """
    local_title = local_track.get("title") or ""
    local_artist = local_track.get("artist") or ""
    sc_full_title = sc_track.get("title") or ""
    sc_artist = sc_track.get("artist") or ""
    sc_real_title = parse_sc_real_title(sc_full_title)

    local_text = f"{local_title} {local_artist}"
    local_bigrams = extract_bigrams(local_text)

    sc_full_bigrams = extract_bigrams(f"{sc_full_title} {sc_artist}")
    sc_real_bigrams = extract_bigrams(f"{sc_real_title} {sc_artist}")

    overlap_full = bigram_overlap_ratio(local_bigrams, sc_full_bigrams)
    overlap_real = bigram_overlap_ratio(local_bigrams, sc_real_bigrams)
    overlap = max(overlap_full, overlap_real)

    if score >= 0.90:
        # Tie-breaker only — don't lower scores
        return score, overlap, 0.0

    if 0.60 <= score < 0.90 and overlap > 0.5:
        blended = 0.80 * score + 0.20 * overlap
        new_score = max(score, blended)
        return new_score, overlap, 1.0 if new_score > score else 0.0
...
def match_track(
    ...
    for idx, tfidf_s in candidates:
        ...
        # EXP4: Levenshtein boost — combined-string only
        score_after_lev, lev_score = levenshtein_boost(local_track, sc, score_after_substr)

        # EXP7: Bigram order boost
        final_score, bigram_overlap, bigram_boosted = bigram_boost(local_track, sc, score_after_lev)
        ...
        if final_score > best_score:
            ...
            best_debug = {
                ...
                "lev_score": lev_score,
                "lev_boosted": score_after_lev > score_after_substr,
                "bigram_overlap": bigram_overlap,
                "bigram_boosted": bool(bigram_boosted),
...
def format_sample(lt: dict, sc: dict | None, score: float, debug: dict) -> str:
    ...
    bb = debug.get("bigram_boosted", False)
    bo = debug.get("bigram_overlap", 0.0)
    ...
    if bb:
        boost_parts.append(f"BIGRAM={bo:.2f}")
    boost_str = f" [{'+'.join(boost_parts)} cont={cont:.3f}]" if boost_parts else ""
    bigram_str = f" bigram={bo:.2f}" if not bb and bo > 0 else ""

    lines = [
        f"  [{score:.3f}] base={base:.3f} {penalty_str}{boost_str}{bigram_str}",
        f"    LOCAL: {local_label}",
        f"    SC:    {sc_label}",
...
def main() -> None:
    ...
    print("\nMatching (exp7: v3 + bigram order boost)...")
    ...
    print(f"  Bigram boosts:      {bigram_count} ({bigram_count / total * 100:.1f}%)\n")
    ...
    print("=" * 60)
    print("CONFIDENCE DISTRIBUTION  [EXP7: BIGRAM ORDER]")
    print("=" * 60)
    ...
    print(f"\n{'=' * 60}")
    print("COMPARISON: v3 baseline vs exp7 bigram")
    print("=" * 60)
    print(f"  {'Track':<45} {'v3':>6}  {'exp7':>6}  {'delta':>7}  boosts")
    print(f"  {'-'*45} {'-'*6}  {'-'*6}  {'-'*7}  {'-'*10}")

    for title_frag, artist_frag, note in spot_checks:
        key = f"{title_frag}|{artist_frag or ''}"
        lt, sc, exp7_s, v2_s, debug = spot_results.get(key, (None, None, 0.0, 0.0, {}))
        label = f"{title_frag}" + (f" ({artist_frag})" if artist_frag else "")
        # v3_s for this spot check is approximate from score_v2 call we stored in v2_s slot
        # Re-label: v2_s slot actually holds score_v2 baseline here, exp7_s is new score
        delta = exp7_s - v2_s
        boosts = []
        if debug.get("substr_boosted"):
            boosts.append("sub")
        ...
        bo = debug.get("bigram_overlap", 0.0)
        print(f"  {label:<45} {v2_s:>6.3f}  {exp7_s:>6.3f}  {delta:>+7.3f}  {boost_str}  bgr={bo:.2f}")
