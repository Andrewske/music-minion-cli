"""EXP8: Filename-as-ground-truth scoring on top of Merged v3.
...
- Separate filename scoring path: cleaned filename stem scored independently
  against SC tracks using containment + substring (no artist-weighting since
  filenames embed artist inline)
- Filename substring boost: if cleaned filename (normalized) is a substring of
  SC title (or vice versa), high confidence — requires only >=3 chars, no
  minimum word count (filenames are closer to original SC titles)
- Final score = max(v3_score, filename_score)
- Tracks which path won for each match

Key insight: local files were downloaded from SoundCloud, so filenames preserve
the ORIGINAL SC title (before Kevin cleaned up metadata). Filename stems like
"Knife Party - Boss Mode [Free Download]" contain the exact SC title.

Run with: uv run python scripts/test_exp8_filename_priority.py
"""
...
def filename_containment_score(
    ...
    """Pure containment score for filename tokens vs SC tokens.

    No artist-weighting since filenames embed artist inline as plain text.
    Applies noise penalty and reverse-remix penalty same as v3.
    """
...
def filename_substring_boost(
    ...
    """Substring boost for filename path.

    Less restrictive than title-only substring:
    - Requires >= 2 meaningful words (guard against single-word false positives)
    - Shorter string: requires >= 3 chars total
    Filenames are closer to original SC titles so this is high-confidence.
    """
    if not filename_clean_norm or len(filename_clean_norm) < 3:
        return fn_score, False

    # Guard: require >= 2 meaningful words to avoid single-word substring noise
    fn_words = [
...
def score_filename_path(
    ...
    """Score filename stem independently against SC candidates.

    Returns (best_sc, best_score, debug_info).
    """
    path = local_track.get("local_path", "")
    if not path:
        return None, 0.0, {}
    ...
    # Guard: require >= 2 meaningful tokens so single-word filenames don't
    # produce false positives via substring matching common words.
    if len(filename_tokens) < 2:
        return None, 0.0, {}

    # Reuse TF-IDF candidates — the index already incorporates filenames
    # but we want to score purely by filename containment, so we still
    # use TF-IDF to get the shortlist efficiently.
...
def match_track_v3_path(
    local_track: dict,
    sc_tracks: list[dict],
    vectorizer: "TfidfVectorizer",
    sc_matrix: "np.ndarray",
) -> tuple[dict | None, float, dict]:
    """Original v3 scoring: title+artist+filename tokens combined."""
...
def match_track(
    ...
    """EXP8: max(v3_score, filename_score) — whichever path is more confident."""
    v3_match, v3_score, v3_debug = match_track_v3_path(
        local_track, sc_tracks, vectorizer, sc_matrix,
    )
    fn_match, fn_score, fn_debug = score_filename_path(
        local_track, sc_tracks, vectorizer, sc_matrix,
    )

    if fn_score > v3_score:
        # Filename path won — annotate debug so we can track it
        merged_debug = {**fn_debug, "path_winner": "filename", "v3_score": v3_score}
        return fn_match, fn_score, merged_debug

    merged_debug = {**v3_debug, "path_winner": "v3", "fn_score": fn_score}
    return v3_match, v3_score, merged_debug
...
def format_sample(lt: dict, sc: dict | None, score: float, debug: dict) -> str:
    ...
    if path_winner == "filename":
        # Filename path won — show filename-specific info
...
def main() -> None:
    ...
    print("\nMatching (EXP8: v3 + filename-as-ground-truth scoring)...")
    ...
    print("\n" + "=" * 60)
    print("CONFIDENCE DISTRIBUTION  [EXP8: FILENAME PRIORITY]")
    print("=" * 60)
    for b in BUCKET_KEYS:
        n = counts[b]
        pct = n / total * 100
        bar = "#" * int(pct / 2)
        # show how many in this bucket came from filename path
        fn_in_bucket = sum(
    ...
    # --- EXP8: Filename path winners ---
    fn_wins = [
        (lt, sc, score, debug)
        for lt, sc, score, debug in results
        if debug.get("path_winner") == "filename"
    ]
    # Sort by score desc to show best filename wins
    fn_wins_sorted = sorted(fn_wins, key=lambda x: x[2], reverse=True)
    print(f"\n{'=' * 60}")
    print(f"EXP8: FILENAME PATH WINNERS — {len(fn_wins)} total, showing top 20")
    print("=" * 60)
    for lt, sc, score, debug in fn_wins_sorted[:20]:
        ...
        print(f"    cleaned:{debug.get('fn_cleaned', '')!r}")
    ...
    for title_frag, artist_frag, note in spot_checks:
        ...
        for lt, sc, score, debug in results:
            ...
            if title_frag.lower() in lt_title and (artist_frag is None or artist_frag.lower() in lt_artist):
                print(format_sample(lt, sc, score, debug))
                # v3_baseline: when filename path won it stored v3_score; otherwise score IS v3
    ...
    print(f"\n{'=' * 60}")
    print("COMPARISON: v3 baseline vs exp8 (filename priority)")
    print("=" * 60)
    print(f"  {'Track':<45} {'v3':>6}  {'exp8':>6}  {'delta':>7}  path/boosts")
    print(f"  {'-'*45} {'-'*6}  {'-'*6}  {'-'*7}  {'-'*12}")

    for title_frag, artist_frag, note in spot_checks:
        key = f"{title_frag}|{artist_frag or ''}"
        lt, sc, exp8_s, v3_s, debug = spot_results.get(key, (None, None, 0.0, 0.0, {}))
        ...
        print(f"  {label:<45} {v3_s:>6.3f}  {exp8_s:>6.3f}  {delta:>+7.3f}  {boost_str}")
