"""EXP9: Enhanced SC title parsing with label/artist prefix extraction.

Base: Strategy Merged v3

Added EXP9:
- parse_sc_title_parts() splits SC title on " - " into segments:
  * 3+ segments: last = real title, middle(s) = parsed artist(s), first = label
  * 2 segments: last = real title, first = parsed artist
  * 1 segment: title only (no parsed artist)
- sc_track_tokens() now returns sc_parsed_artist_tokens from middle segments
- containment_score_with_penalty() applies 2x weight to sc_parsed_artist_tokens
  when local artist tokens match them (same weight as EXP3 sc_artist_tokens)
- Statistics: how many SC tracks have parseable artist in title, how many
  matches improved

Run with: uv run python scripts/test_exp9_title_prefix.py
"""
...
def parse_sc_title_parts(sc_title: str) -> tuple[str, list[str], str | None]:
    """Parse SC title into (real_title, parsed_artists, label).

    Segments split on " - ":
    - 1 segment:  real_title=seg[0], parsed_artists=[], label=None
    - 2 segments: real_title=seg[1], parsed_artists=[seg[0]], label=None
    - 3+ segments: real_title=seg[-1], parsed_artists=seg[1:-1], label=seg[0]

    Returns (real_title, parsed_artists, label).
    """
    cleaned = strip_bracket_suffixes(sc_title)
    segs = [s.strip() for s in cleaned.split(" - ")]
    if len(segs) == 1:
        return segs[0], [], None
    if len(segs) == 2:
        return segs[1], [segs[0]], None
    # 3+: first=label, middle=artists, last=title
    return segs[-1], segs[1:-1], segs[0]
...
def sc_track_tokens(track: dict) -> tuple[set[str], set[str], set[str], set[str]]:
    """Returns (all_tokens, real_title_tokens, sc_artist_tokens, sc_parsed_artist_tokens).

    sc_parsed_artist_tokens: tokens extracted from middle segments of "Label - Artist - Title" titles.
    """
    title = track.get("title") or ""
    artist = track.get("artist") or ""

    full_tokens = tokenize(f"{title} {artist}")
    real_title, parsed_artists, _label = parse_sc_title_parts(title)
    real_tokens = tokenize(f"{real_title} {artist}")
    sc_artist_tokens = tokenize(artist) if artist else set()

    # Parsed artist tokens from title middle segments
    sc_parsed_artist_tokens: set[str] = set()
    for pa in parsed_artists:
        sc_parsed_artist_tokens |= tokenize(pa)

    combined = full_tokens | real_tokens
    return combined, real_tokens, sc_artist_tokens, sc_parsed_artist_tokens
...
def containment_score_with_penalty(
    ...
    sc_parsed_artist_tokens: set[str] | None = None,
) -> tuple[float, dict]:
    if sc_parsed_artist_tokens is None:
        sc_parsed_artist_tokens = set()
    ...
    if not local_tokens:
        return 0.0, {
            ...
            "artist_missed": set(), "parsed_artist_matched": set(),
        }
    ...
    # --- EXP9: extend used_sc_tokens with parsed artist tokens for scoring ---
    # Parsed artists from middle segments get 2x weight like EXP3 sc_artist_tokens.
    # We track which local artist tokens matched via parsed title artists.
    parsed_artist_matched = local_artist_tokens & sc_parsed_artist_tokens

    # --- Artist-weighted base score (EXP3 + EXP9) ---
    non_artist_local = local_tokens - local_artist_tokens
    # Artist tokens: matched via sc_tokens OR sc_parsed_artist_tokens
    all_sc_for_artist = used_sc_tokens | sc_parsed_artist_tokens
    artist_matched = local_artist_tokens & all_sc_for_artist
    artist_missed = local_artist_tokens - all_sc_for_artist
    ...
    # --- Artist boost with guard (EXP3 + EXP9 + fix) ---
    # Only apply 1.1x when non-artist token coverage > 0.5
    # Also fires when local artist matches sc_parsed_artist_tokens
    artist_boost = 1.0
    non_artist_coverage = len(non_artist_matched) / len(non_artist_local) if non_artist_local else 0.0
    all_sc_artist = used_sc_tokens | sc_artist_tokens | sc_parsed_artist_tokens
    if local_artist_tokens and local_artist_tokens.issubset(all_sc_artist):
    ...
    debug = {
        ...
        "parsed_artist_matched": parsed_artist_matched,
    }
    return final, debug
...
def match_track(
    ...
    for idx, tfidf_s in candidates:
        sc = sc_tracks[idx]
        sc_tokens_all, sc_real_tokens, sc_artist_tokens, sc_parsed_artist_tokens = sc_track_tokens(sc)

        containment_s, score_debug = containment_score_with_penalty(
            local_tokens, critical_tokens, sc_tokens_all, sc_real_tokens,
            local_artist_tokens, sc_artist_tokens, sc_parsed_artist_tokens,
        )
        ...
        if final_score > best_score:
            ...
            best_debug = {
                ...
                "parsed_artist_matched": score_debug["parsed_artist_matched"],
                "sc_parsed_artist_tokens": sc_parsed_artist_tokens,
            }
...
def main() -> None:
    ...
    # --- EXP9: pre-compute SC title segment stats ---
    sc_with_2seg = sum(
        1 for t in sc_tracks
        if " - " in strip_bracket_suffixes(t.get("title") or "")
        and len((strip_bracket_suffixes(t.get("title") or "")).split(" - ")) == 2
    )
    sc_with_3plus_seg = sum(
        1 for t in sc_tracks
        if len((strip_bracket_suffixes(t.get("title") or "")).split(" - ")) >= 3
    )
    print(f"\nSC title segment analysis:")
    print(f"  2-segment (Artist - Title): {sc_with_2seg} ({sc_with_2seg / len(sc_tracks) * 100:.1f}%)")
    print(f"  3+ segment (Label - Artist - Title): {sc_with_3plus_seg} ({sc_with_3plus_seg / len(sc_tracks) * 100:.1f}%)")
    print(f"  Total with parseable artist: {sc_with_2seg + sc_with_3plus_seg} ({(sc_with_2seg + sc_with_3plus_seg) / len(sc_tracks) * 100:.1f}%)")

    print("\nMatching (exp9: v3 + title prefix artist extraction)...")
    results: list[tuple[dict, dict | None, float, dict]] = []
    substr_count = 0
    lev_count = 0
    parsed_artist_boost_count = 0
    ...
    for i, lt in enumerate(local_tracks):
        ...
        if debug.get("lev_boosted"):
            lev_count += 1
        if debug.get("parsed_artist_matched"):
            parsed_artist_boost_count += 1
    ...
    print(f"  Parsed-artist matches (EXP9): {parsed_artist_boost_count} ({parsed_artist_boost_count / total * 100:.1f}%)\n")
    ...
    print("=" * 60)
    print("CONFIDENCE DISTRIBUTION  [EXP9: title prefix artist]")
    print("=" * 60)
    ...
    print(f"\n{'=' * 60}")
    print("COMPARISON: v2 baseline vs exp9")
    print("=" * 60)
    print(f"  {'Track':<45} {'v2':>6}  {'e9':>6}  {'delta':>7}  boosts")
    print(f"  {'-'*45} {'-'*6}  {'-'*6}  {'-'*7}  {'-'*10}")

    for title_frag, artist_frag, note in spot_checks:
        key = f"{title_frag}|{artist_frag or ''}"
        lt, sc, e9_s, v2_s, debug = spot_results.get(key, (None, None, 0.0, 0.0, {}))
        ...
        print(f"  {label:<45} {v2_s:>6.3f}  {e9_s:>6.3f}  {delta:>+7.3f}  {boost_str}")

    # --- EXP9: show spot-check examples where parsed_artist_matched fired ---
    print(f"\n{'=' * 60}")
    print("EXP9: TRACKS WHERE PARSED ARTIST FROM TITLE HELPED")
    print("=" * 60)
    pa_examples = [
        (lt, sc, score, debug) for lt, sc, score, debug in results
        if debug.get("parsed_artist_matched")
    ]
    print(f"  Total matches using parsed artist from SC title: {len(pa_examples)}")
    for lt, sc, score, debug in pa_examples[:15]:
        sc_title = (sc or {}).get("title", "") if sc else ""
        _, parsed_arts, label_seg = parse_sc_title_parts(sc_title)
        lt_artist = lt.get("artist") or "?"
        lt_title = lt.get("title") or "?"
        pa_matched = debug.get("parsed_artist_matched", set())
        print(f"  [{score:.3f}] local: {lt_artist} - {lt_title}")
        print(f"          sc:    {sc_title}")
        print(f"          label={label_seg!r}  parsed_artists={parsed_arts}  matched={sorted(pa_matched)}")
